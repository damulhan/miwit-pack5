...
if (preg_match("/\.($config[cf_movie_extension])$/i", $file[0]['file'])) {
    ...
    $file[0]['movie'] = true;
	#error_log('000'. $row['content']);
}
elseif ($file[0]['view']) {
	#error_log('11111111111'. $file[0]['view']);
	...
	#error_log('11111111111'. $file[0]['view']);
	#error_log('111'. $row['content']);

	#$_content = $row['content'];
}

#$row['content'] .= print_r($file, true);
#$row['content'] .= '이게워';
#error_log('111');
#error_log(print_r($file, true));
#error_log('222'. $row['content']);
#error_log('333'. $_content);
#$row['content'] = $_content;
#error_log('444'. $row['content']);
