...
<!-- 게시판 목록 시작 -->
<table width="<?= $bo_table_width ?>" align="center" cellpadding="0" cellspacing="0">
    <tr>
        <td id=mw_basic>
            ...
            <form name="fboardlist" id="fboardlist" method="post">
                ...
                <table width=100% border=0 cellpadding=0 cellspacing=0>
                    <tr>
                        <td colspan=<?= $colspan ?> height=1 class=mw_basic_line_color></td>
                    </tr>
                    <? if ($mw_basic['cf_type'] != "gall" && !$mw_basic['cf_social_commerce'] && !$mw_basic['cf_talent_market']) { ?>
                        ...
                        <tr>
                            <td colspan=<?= $colspan ?> height=3 style="background-color:#efefef;"></td>
                        </tr>
                        <tr>
                            <td colspan=<?= $colspan ?> height=3 style=""></td>
                        </tr>
                    <? } ?>
                    <? if ($mw_basic['cf_type'] == "gall") { ?> <tr>
        ...
        */
                            ...
                            if ($thumb_size[0]) {
                                if ($thumb_size[0] != $set_width || $thumb_size[1] != $set_height) {
                                    ...
                                    mw_make_thumbnail(
                                        $mw_basic['cf_thumb_width'],
                                        $mw_basic['cf_thumb_height'],
                                        $thumb_file,
                                        $thumb_file,
                                        $mw_basic['cf_thumb_keep'],
                                        $list[$i]['wr_datetime']
                                    );
                    ...
                    ?>
                        ...
                        --><img src="<?= $thumb_file ?>" class="thumb" <?php if (!$mw_basic['cf_thumb_keep'])
        ...
        --><? if ($list[$i]['icon_new']) {
                                        echo "<div class='icon_gall_new'><img src='{$board_skin_path}/img/icon_gall_new.png'></div>";
                                    } ?><div><a href="<?= $list[$i]['href'] ?>"><img src="<?= $thumb_file ?>" width=<?= $mw_basic['cf_thumb_width'] ?> height=<?= $mw_basic['cf_thumb_height'] ?> align="absmiddle" class="list_thumb_img"></a></div><!--
    --></td>
                                <?php } ?>
        ...
        <?php } */ ?>
            ...
            <? } */ ?>
                            ...
                            ?>
                                ...
                                <tr>
                                    <td colspan=<?= $colspan ?> height=1 class="mw_basic_list_line"></td>
                                </tr>
                            <? } ?>
                        <? } ?>
                        <? $line_number++; ?>
    ...
    if ($rest > 0) {
        ...
        echo "</tr>\n";
                    ...
                    <?php if ($line_number == 0) {
                        echo "<tr><td colspan={$colspan} class=mw_basic_nolist>게시물이 없습니다.</td></tr>";
                    } ?>
                    <tr>
                        <td colspan=<?= $colspan ?> class=mw_basic_line_color height=1></td>
                    </tr>
                </table>
            ...
            <div class=mw_basic_page>
                <?php if ($prev_part_href) {
                    echo "<a href='$prev_part_href'>이전검색</a>";
                } ?>
                <?php echo $write_pages; ?>
                <?php if ($next_part_href) {
                    echo "<a href='$next_part_href'>다음검색</a>";
                } ?>
            </div>
...
<style>
    <?php if ($mw_basic['cf_type'] == 'desc') echo "#mw_basic .list_desc_info { display:block; }" . PHP_EOL; ?>@media screen and (max-width:500px) {
