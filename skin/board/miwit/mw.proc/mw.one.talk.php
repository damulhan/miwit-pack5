    ...
    <form name="fwrite" method="post" onsubmit="return fwrite_check(document.fwrite);" enctype="multipart/form-data">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <colgroup width=100>
                <colgroup width=''>
                <tr>
                    <td colspan=2 height=2 class=mw_basic_line_color></td>
                </tr>
        </table>
                        ...
                        <tr>
                            <td valign="top">
                                ...
                                <? if (($is_member && $member['mb_id'] == $member['mb_id'] && !$member['wr_anonymous']) || $is_admin) { ?>
                                    <div style="margin:0 0 0 10px;"><a href="javascript:mw_member_photo('<?= $member['mb_id'] ?>')" style="font:normal 11px 'gulim'; color:#888; text-decoration:none;"><? echo $is_comment_image ? "사진변경" : "사진등록"; ?></a></div>
                                <? } ?>
                            ...
                            <td valign="top">
                                ...
                                <? if (!$is_dhtml_editor || $mw_basic['cf_editor'] != "cheditor") { ?>
                                    <textarea id="wr_content" name="wr_content" style='width:100%; word-break:break-all;' rows=5 itemname="내용" required class=mw_basic_textarea <? if ($is_dhtml_editor && $mw_basic['cf_editor'] == "geditor") echo "geditor"; ?> <? if ($write_min || $write_max) { ?>onkeyup="check_byte('wr_content', 'char_count');" <? } ?>><?= $content ?></textarea>
                                    <? if (($write_min || $write_max) && !$is_dhtml_editor) { ?><script type="text/javascript">
...
<? } else { ?>
    ...
    --></td>
        ...
        <? if (!$mw_basic['cf_post_name']) { ?>
            <? if ($mw_basic['cf_attribute'] != "anonymous") { ?> <td>
                    <nobr class=mw_basic_list_name><?= $list[$i]['name'] ?></nobr>
                </td> <? } ?> <? } ?>
    ...
    ?>
        ...
        <tr>
            <td colspan=<?= $colspan ?> height=1 style="border-top:1px dotted #e7e7e7"></td>
        </tr>
    <? } ?>
    <? $line_number++; ?>
<? } ?>
